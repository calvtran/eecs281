...
String& String::erase(size_t pos, size_t len) {
    ...
    if (begin != 0) {
        if (end > sz) {     // erase end
            for (size_t i = pos; cstr[i] != '\0'; ++lenActual, ++i) {}
            cstr[pos] = a_null_byte;
            sz -= lenActual;
        }
        else {
            for (; ; ++begin, ++end, ++lenActual) {     // erase middle
                cstr[begin] = cstr[end];
                if (cstr[end] == '\0') {
                    break;
                }
        ...
        else {                  // erase beginning
            *this = (*this).substr(len, sz);
        }
    }
...
String& String::insert(size_t pos, const String& str) {
    ...
    size_t endlen = sz - pos + 1;

    // expand
    String tempA = (*this).substr(0, pos);
    String in = str;
    String tempB = (*this).substr(pos, endlen);
    *this = tempA;
    *this += in;
    *this += tempB;

    sz = tempA.size() + in.size() + tempB.size();
